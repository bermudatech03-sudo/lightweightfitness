...
class PushSubscription(models.Model):
    ...
    Admin/staff subscriptions are linked to their account (`user`). Member
    subscriptions are linked to `member` instead (Phase 2 — admin scans a QR
    generated by the member's browser and links it from that member's profile).
    Exactly one of `user`/`member` should be set, never both.
    """
    ...
    def __str__(self):
        who = self.user.username if self.user_id else (self.member.name if self.member_id else "unlinked")
...
class Notification(models.Model):
    ...
    # Explicit link to who this is about — set at creation time wherever the
    # Member object is already in hand (e.g. absent/diet_reminder). Not every
    # trigger has one (admin-facing/staff/enquiry ones leave it null).
    # SET_NULL rather than CASCADE: this table is a history/audit log — a
    # deleted member shouldn't erase their past notification records.
